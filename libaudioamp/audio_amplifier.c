...
static int amp_set_output_devices(amplifier_device_t *device, uint32_t devices) {
    ...
    if (is_speaker(devices)) {
        if (!tfa9890->tfa_enabled) {
            ALOGV("turning speaker on for speaker device %d", devices);
            tfa9890->tfa_enabled = 1;
            tfa9890_EQset(0);
            audio_smartpa_enable(1);
        }
    } else if (tfa9890->tfa_enabled) {
        ALOGV("turning speaker off for output device %d", devices);
        audio_smartpa_enable(0);
        tfa9890->tfa_enabled = 0;
    }
